...
def get_unet_mobilenetv2_reg(input_shape=(288, 512, 3), max_lanes=4, l2_reg=4e-4):
    ...
    x = UpSampling2D(size=(2, 2), interpolation='bilinear')(x)
    x = SeparableConv2D(32, 3, padding='same', use_bias=False,
                        depthwise_regularizer=l2(l2_reg), pointwise_regularizer=l2(l2_reg))(x)
    x = BatchNormalization()(x)
    x = ReLU()(x)
    ...
    x = UpSampling2D(size=(2, 2), interpolation='bilinear')(x)
    x = SeparableConv2D(16, 3, padding='same', use_bias=False,
                        depthwise_regularizer=l2(l2_reg), pointwise_regularizer=l2(l2_reg))(x)
    x = BatchNormalization()(x)
    x = ReLU()(x)
    
    mask_out = Conv2D(1, 1, activation='sigmoid', name='mask_out', kernel_regularizer=l2(l2_reg))(x)
    
    model = Model(inputs, mask_out, name="MobileNetV2_UNet_Reg")
    return model
